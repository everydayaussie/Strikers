...
internal static partial class Program
{
    ...
    private static string BothReadyInstructions(Lobby lobby, IEnumerable<Preset> presets)
    {
        ...
        Stand in the Machine Strike menu, on the challenge list, and run: write
                                                              ...
                                                              Preset? preset = null)
    {
        ...
        var ownFallback = lobby.IsHost ? preset : null;
        var boardToWrite = lobby.Board ?? (ownFallback is { Board.Count: > 0 } p2 ? p2.Board : null);
        var victoryToWrite = lobby.VictoryPoints != Preset.RuleNotSet
            ? lobby.VictoryPoints
            : ownFallback?.VictoryPoints;
        var draftToWrite = lobby.DraftPoints != Preset.RuleNotSet
            ? lobby.DraftPoints
            : ownFallback?.DraftPoints;

        var depthToWrite = lobby.PlacementRows != Preset.RuleNotSet
            ? lobby.PlacementRows
            : (int?)null;
    ...
    public static string[] NameArgs(string? mine, string? theirs)
    {
        var me = Names.Clean(mine);
        var them = Names.Clean(theirs);
        if (me.Length == 0 && them.Length == 0)
        {
            return [];
        }
    ...
    private static int LobbyWrite(Lobby lobby, string probe, bool yes, bool interactivePlacement = false)
    {
        if (!lobby.BothReady)
        {
        ...
        if (yes && RunProbeQuiet(probe, ["--match-live"], 2) == 0)
        {
            Console.Error.WriteLine("  the game is still in a match, or on its victory screen. Press Continue in " +
                                    "the game, stand on the challenge list, then Set up the match again. " +
                                    "Nothing was written.");
            return 1;
        }

        foreach (var (what, a) in WriteSteps(lobby, remote, interactivePlacement, _appliedPreset))
        {
            ...
            var code = a.Contains("--restore-when-live")
                ? RunProbeUntilArmed(probe, out _, full)
                : RunProbeQuiet(probe, full);
            if (code == 0)
            {
                continue;
            }

            Console.Error.WriteLine($"  the step was: {probe} {string.Join(' ', full)}");
            Console.Error.WriteLine($"\n  live-probe exited {code}. Stopping, the later steps did not run, " +
        ...
        var step2 = interactivePlacement
            ...
            : $"Place your own machines by hand on: {string.Join(", ", lobby.Local.Placements.Select(p => $"({p.X},{p.Y}) dir {p.Dir}"))}";
    ...
    private static string ReadBuild(string probe)
    {
        try
        {
            var psi = new System.Diagnostics.ProcessStartInfo(probe)
